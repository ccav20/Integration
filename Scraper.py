...
from LinkedInScraping_master.utils import Profile, ScrapingException, is_url_valid, HumanCheckException, \
    wait_for_loading, wait_for_scrolling, \
...
class Scraper(Thread):
    ...
    def run(self):
        ...
        if not self.browser.current_url == "https://www.linkedin.com/feed/":
            if 'https://www.linkedin.com/checkpoint/challenge' not in self.browser.current_url:
    ...
    def scrape_profile(self, linkedin_url, waiting_time=45):
        ...
        try:
            ...
            print(f"Another try will be performed within {waiting_time} seconds...")
    ...
    def __scrape_profile(self, profile_linkedin_url):
        ...
        print('self.browser.current_url ======>   ', str(self.browser.current_url).strip())
    ...
    def scrape_profile_name(self):
        name_result_from_js = self.browser.execute_script(
            "return document.getElementsByClassName('display-flex justify-space-between pt2')[0].children[0].innerText")

        return name_result_from_js
    ...
    def scrape_email(self):
        ...
        try:
            email = self.browser.execute_script('''return (function () {
            var jobs =[];
            var els = document.getElementsByClassName('pv-contact-info__ci-container t-14');
            for (var i = 0; i < els.length; i++) {
            try{
                if (els[i].innerText.includes('@')) {
...
;)();
        '''
            ...
            print('email from scraper ======>  ', email)
    ...
    def scrape_jobs(self):
                        ...
                        if (els[i].className != 'pv-entity__position-group-role-item-fading-timeline') {
                            if (els[i].getElementsByClassName('pv-entity__position-group-role-item-fading-timeline').length > 0) {} else {
                                ...
                                try {
                                    ...
                                    try{
                                    company_name = els[i].getElementsByClassName('pv-entity__company-summary-info')[0].getElementsByTagName('h3')[0].innerText;
                                }catch (err) {
                                        company_name = ''
                                    }
                                }
        ...
        for job in clean_jobs:
            company_industry, company_employees, company_domain = self.scrape_company_details(job[2])
    ...
    def scrape_company_details(self, company_url):
        ...
        print('linkedin da company ====> ', company_url)

        try:
            company_domain = self.browser.execute_script("return document.getElementsByClassName("
                                                         "'org-contact-info__content "
                                                         "ph2 pb2')[0].innerText")
            print('url atual ===> ', self.browser.current_url)
            print('company_domain scrapper ====> ', company_domain)
        except WebDriverException:
            company_domain = ''
        ...
        return company_industry, company_employees, company_domain
